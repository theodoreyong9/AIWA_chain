...
**Assumed:** collision-resistant hashing, EUF-CMA-secure signatures, deterministic serialization.
**Not assumed:** a synchronized wall clock, continuous access to any external chain, a global consensus quorum, a globally replicated state.
...
**Reception monotonicity.** For domain $D$ observing $X$: $\mathrm{seen}_D(X, e_{i+1}) \geq \mathrm{seen}_D(X, e_i)$ for successive real commitments $e_i$. A claim of having seen less than previously committed is rejected.
...
valid only if causally chained to $D$'s own last accepted transition and carrying a real sequential proof (§6). Progression is local: $\mathrm{epoch}_A$ and $\mathrm{epoch}_B$ are never directly comparable.
...
**Identity-cost, now incremental too — the last of the three real costs above, closed.** The identical, real fix, verified the same way (byte-identical to a full replay). `state-snapshot.js`'s own real snapshot now covers all three — wallet, Mirror, and identity-cost — together, with the identical, real legacy-detection discipline extended one tier further: a real, mirror-only snapshot (saved before identity-cost joined this mechanism) reports identity-cost as genuinely unknown, never an empty state, triggering one real, final full catch-up for that one part alone.

**Real caution found while fixing this.** `topoOrder()`'s own real sort order for causally-independent events follows their own content-addressed id — never insertion order. An early, real draft of the identity-cost equivalence check assumed the first element returned was genesis; it wasn't. Caught before being written into anything permanent — worth stating plainly as a real, general trap for any future incremental-catch-up code written against a real `EventDag`.
...
**Domain invariance.** For identical evidence, $\hat\theta_X$ is identical regardless of which domain computes it or via which path evidence arrived — a consequence of event ids already being content-addressed (§3): identical causal content produces an identical id independent of transport.
...
**Attestation.** A two-hop signature chain: an origin domain signs issuance of a hardware root; the root's own key signs its binding to a target domain. Both independently verified. $\geq 2$ distinct, independently-issued roots required — one root moves trust to a single unit, never establishes a minimum.
...
**Verified, not only asserted.** `matching-contract.js` is a real, second, working contract, composing directly with this one — a real third party pre-commits to also sending a bonus if a specific, real offer above resolves as a win, verified by directly calling this contract's own real, exported `resolveGenerousSend` (no shared execution environment exists here, §1: composing can only ever mean a real, ordinary function import, never a special "call" mechanism). $\mathrm{wrapsContractId}$ is real, signed transparency; the real security comes from the hardcoded import itself, never from that field alone. Verified end to end through wallet.js's own generic mechanism (§15.2): a real win in the wrapped contract produces a real, valid composed payout that genuinely moves real, spendable AIWA; a real, honest loss genuinely propagates, refusing the composed payout with no special case in either contract's own file.

**Real interface.** The "Give" tab's own "Match someone else's offer" card sends a real match commitment referencing any real, wrapped offer's own event id. Resolution happens at the identical, real moment the wrapped offer itself does — the same real progression loop that resolves an offer also checks for any real, pending matches referencing it, reusing the identical, just-computed real proof rather than a separately re-derived one (`matching-contract-scan.js`, pure, directly tested).

**The real collision risk, structurally closed, not merely documented.** A real signature check alone only ever proves "signed by this key, over this exact content" — never "this is really the trusted module it claims to be." Verified concretely: a forged commitment claiming $\mathrm{aiwa\text{-}generous\text{-}transfer\text{-}v1}$ passes its own real signature check just fine, from a completely different, real keypair. `registerVerifiedContract` (§16) makes registration itself demand proof: a contract's own currently-deployed real source is independently re-hashed against a real, pinned expected value before its `verifyPayout` is ever added to the real registry — a mismatch refuses that one registration outright, never silently trusted from a name. The reference application wires this in for every real contract it registers, from the first one onward — not deferred until some hypothetical future feature might one day accept third-party contracts; the discipline is real and active now.
...
### 16.1 A real, asymmetric trust direction for interchain verification — confirmed, not merely asserted
