...
\textbf{Honest limit, current state.} No interface exists yet --- publishing and reading a contract-spec is only ever done by code today, real and tested, never through Continuum, Ignition, or Mirror. A genuine proof of concept: the mechanism is real and safe; only the interface to reach it is missing. Real transport (\S11.1's own P2P sync, unchanged) already carries a \code{contract-spec} event exactly like any other --- the missing piece is narrower than transport.
...
Generous transfer, deterministic (\S15) & \code{public/core/generous-transfer.js} --- never chance, verified end-to-end against the real progression protocol \\
