...
\emph{Computable form:} $\beta(1-T) \ln A + \ln(1 + C/A^{\beta(1-T)}) \equiv \ln(A^{\beta(1-T)} + C)$, represented in the left form to bound overflow for large $A$.

\textbf{Not yet wired.} Unlike $t$ and $A$, $T$ \emph{is} accepted directly from an \code{accrual} event's own payload --- carried forward unchanged on a later event for the same domain that omits it, defaulting to $0$ if never set at all. The reference application's own \code{ignition.js}, the only real place a real \code{accrual} event is ever built, never sets $T$ --- every real position's $T$ is $0$ in the app as currently shipped. The $(1-T)$ term itself is real and unit-tested directly against $R$; a real, user-facing patience control remains undone work.

\subsection{Reproducibility across runtimes (Q128 fixed point)}
\label{sec:repro}

$t^{\alpha}$, $A^{\beta(1-T)}$, and the outer $\ln(\cdot)^{\gamma}$ are fractional-exponent and transcendental operations. IEEE 754 guarantees $+, -, \times, \div$ agree bit-for-bit across runtimes; it guarantees nothing for a fractional-exponent \code{pow} or a \code{log} --- two different \code{libm} builds may legitimately disagree in the last bit, and $R$'s own output funds a real, on-chain claim (\S9).

\code{fixed-point-math.js} computes \code{ln}, \code{exp}, and fractional \code{pow} from nothing but BigInt $+, -, \times, \div$, in Q128 binary fixed point (128 fractional bits): a fixed, hardcoded series-term count for each (never ``until convergence,'' itself runtime-variable), exact IEEE-754 double decomposition at the Number$\leftrightarrow$Fixed boundary, and one rule enforced throughout --- never bit-shift a negative BigInt, so no two runtimes' BigInt implementations have to agree on how that case behaves. \code{rewardFixed} is the reproducible Q128 BigInt core; \code{reward()}, the wrapper every existing caller already uses, is unchanged in signature and behavior, converting to/from a plain Number only at its own outer boundary --- a JS-only convenience, never part of the reproducibility guarantee.

\textbf{A structural side effect, not merely a workaround.} $A^{\beta(1-T)}$ is now an arbitrary-precision BigInt rather than a capped-precision float --- the original float formula could in principle reach $\infty$ before $\ln$ ever ran, for a sufficiently large $A$ or $\beta$; the fixed-point version cannot overflow that way for any real parameter combination.

\textbf{Verified concretely, cross-runtime.} An independently-written Rust port of the identical Q128 arithmetic produces the same BigInt, digit for digit, for real test vectors --- including a full year of continuous progression ($3976466040673248597032750975172588536399082$ for a basic case, $70070162968303128449516048132042380811409136$ after $\sim$112M epochs) --- part of \code{tests/rust-interop.test.mjs} alongside every other cross-runtime check (\S\ref{sec:content-addressing}, \S16.1).
...
$1\ \mathrm{AIWA} = 10^{18}$ base units, always integer. Reward's fractional exponents are computed in Q128 fixed-point BigInt arithmetic (\S\ref{sec:repro}), never in floating point internally. \code{accrual.js}'s own real claim path converts that Fixed BigInt straight to base units (\code{fixedToUnits}), never through a float. \code{reward()} itself, kept for existing plain-Number callers, still crosses to and from a float only at its own outer boundary --- a JS-only convenience, never part of the reproducibility guarantee (\S\ref{sec:repro}).
...
\textbf{Identity-cost, now incremental too --- the last of the three real costs above, closed.} The identical, real fix, verified the same way (byte-identical to a full replay). \code{state-snapshot.js}'s own real snapshot now covers all three --- wallet, Mirror, and identity-cost --- together, with the identical, real legacy-detection discipline extended one tier further: a real, mirror-only snapshot (saved before identity-cost joined this mechanism) reports identity-cost as genuinely unknown, never an empty state, triggering one real, final full catch-up for that one part alone.

\textbf{Real caution found while fixing this.} \code{topoOrder()}'s own real sort order for causally-independent events follows their own content-addressed id --- never insertion order. An early, real draft of the identity-cost equivalence check assumed the first element returned was genesis; it wasn't. Caught before being written into anything permanent --- worth stating plainly as a real, general trap for any future incremental-catch-up code written against a real \code{EventDag}.
...
\textbf{Verified, not only asserted.} \code{matching-contract.js} is a real, second, working contract, composing directly with this one --- a real third party pre-commits to also sending a bonus if a specific, real offer above resolves as a win, verified by directly calling this contract's own real, exported \code{resolveGenerousSend} (no shared execution environment exists here, \S1: composing can only ever mean a real, ordinary function import, never a special ``call'' mechanism). $\mathrm{wrapsContractId}$ is real, signed transparency; the real security comes from the hardcoded import itself, never from that field alone. Verified end to end through wallet.js's own generic mechanism (\S15.2): a real win in the wrapped contract produces a real, valid composed payout that genuinely moves real, spendable AIWA; a real, honest loss genuinely propagates, refusing the composed payout with no special case in either contract's own file.

\textbf{Real interface.} The ``Give'' tab's own ``Match someone else's offer'' card sends a real match commitment referencing any real, wrapped offer's own event id. Resolution happens at the identical, real moment the wrapped offer itself does --- the same real progression loop that resolves an offer also checks for any real, pending matches referencing it, reusing the identical, just-computed real proof rather than a separately re-derived one (\code{matching-contract-scan.js}, pure, directly tested).

\textbf{The real collision risk, structurally closed, not merely documented.} A real signature check alone only ever proves ``signed by this key, over this exact content'' --- never ``this is really the trusted module it claims to be.'' Verified concretely: a forged commitment claiming $\mathrm{aiwa\text{-}generous\text{-}transfer\text{-}v1}$ passes its own real signature check just fine, from a completely different, real keypair. \code{registerVerifiedContract} (\S16) makes registration itself demand proof: a contract's own currently-deployed real source is independently re-hashed against a real, pinned expected value before its \code{verifyPayout} is ever added to the real registry --- a mismatch refuses that one registration outright, never silently trusted from a name. The reference application wires this in for every real contract it registers, from the first one onward --- not deferred until some hypothetical future feature might one day accept third-party contracts; the discipline is real and active now.
...
\subsection{A generic scanning primitive for contract-own state}

Tracking a contract's own pending/resolved state from its own DAG events is a second place per-contract hardcoding was tried informally and found not to scale --- the same shape of problem \S15.2 already closed for payouts, here for state. \code{generous-send-scan.js} and \code{matching-contract-scan.js} had independently reimplemented the identical scan --- every id already consumed as a parent of a \code{progression} event, optionally restricted to one domain or to a candidate set --- byte for byte in two of the three places it appeared; \code{matching-contract-scan.js} and \code{relative-rate-scan.js} had independently reimplemented a second pattern, grouping matching events by a derived key.

\code{public/core/contract-scan.js} factors both into two real, generic primitives: \code{collectProgressionParentIds(events, \{domain, only\})} and \code{groupEventsByKey(events, \{predicate, keyOf, itemOf\})}. The three existing scan files now delegate to them; behavior is unchanged --- all 13 pre-existing scan tests pass unmodified, plus 9 new tests for the shared primitives themselves. A future contract needing state richer than pending/resolved starts from these, rather than reinventing the pattern.
...
\subsection{A real, asymmetric trust direction for interchain verification --- confirmed, not merely asserted}
...
\textbf{Confirmed concretely, not left as a reasoned claim.} Every real primitive an external adapter would actually need is independently reproduced and verified in \code{interop/rust-vdf/} (\S16, extended for this): SHA-256 canonicalization (\S3.1), the practical, \emph{cheap} Wesolowski verification (\S6.1) --- the one an external, gas-constrained chain would genuinely use, never the raw symmetric chain, prohibitively expensive to redo --- including its own real prime-derivation and Miller-Rabin primality test, and Ed25519 signature verification, checked against a real, independent Rust library (\code{ed25519-dalek}) rather than the real JS one this project actually uses (\code{@noble/curves}) --- confirming the \emph{algorithm}, never one particular implementation, is what a real signature depends on. This is what ``no shared execution environment, no adapter dependency'' means concretely: a real, external verifier, in a real, different language, using real, different libraries, produces byte-identical answers. The reward formula's own Q128 fixed-point core (\S\ref{sec:repro}) is verified the identical way, added to \code{tests/rust-interop.test.mjs} alongside the checks above.

\textbf{What this confirms, and what it still doesn't.} This is the real, structural readiness for interchain adapters --- not an adapter itself. No specific target chain's own contract language (Solidity, Move, or otherwise) has been chosen or implemented against; that remains real, deliberate future work, left to whichever chain chooses to build it, entirely on its own terms. What this section confirms is narrower and more load-bearing: the verification algorithm itself carries no hidden JS-specific dependency that would make such an adapter infeasible or need this project's own cooperation to build.
...
Accrual formula (\S7) & \code{public/core/reward.js}, \code{public/core/fixed-point-math.js} --- \code{rewardFixed()}'s own reproducible Q128 core (\S\ref{sec:repro}) \\
...
Contract composability, verified (\S15.1) & \code{public/core/matching-contract.js} --- a real, second contract, composing directly with generous-transfer.js \\
Generic contract-state scanning (\S15.3) & \code{public/core/contract-scan.js} --- factored out of three independently duplicated scan implementations \\
