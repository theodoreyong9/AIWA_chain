...
\title{\textbf{AIWA Yellow Paper}\\[4pt]\large Causal Coordination and Local Value Accrual for Partition-Tolerant Networks}
\author{}
\date{Version 1.1 --- reference implementation specification}
...
\textbf{Causal coordination} derives a deterministic causal layer for authenticated events from a common genesis, causal history, and Mirror observations. Independent domains continue operating while disconnected and reconcile later, when evidence becomes available.

\textbf{Accrual} provides controlled, \emph{local} creation of value. An identity is activated through an externally anchored Genesis Commitment (a real Solana burn). Its claimable value then evolves according to a deterministic progression function, driven by real sequential computation, not a shared clock. Claiming converts accrued value into native AIWA balance and resets the domain's own patience clock. Once claimed, AIWA transfers between wallets through authenticated causal history, with no continued dependency on the external activation chain.

\begin{quote}
\textbf{Progression determines how much value a domain may claim.\\
...
\[
\mathrm{domain}(i) = \mathrm{SHA\text{-}256}(\mathrm{publicKey}_i)
\]
...
A commitment is either \code{empty} (nothing received this epoch) or \code{full} (a real, verifiable set of references to source events). Reception monotonicity holds: a domain's own successive claims about what it has seen of another domain must never claim to have seen \emph{less} than it previously, verifiably claimed.
...
A domain's own progression epoch advances only through a valid transition: exactly $+1$, causally chained to the domain's own last accepted transition, and carrying a real sequential proof (\S\ref{sec:sequential-proof}).
...
This computation is always local to a single, real device --- never distributed across the network, never something other domains help compute. Whatever device currently holds and runs a domain's own keypair is the one real device advancing that domain's own next epoch, right now; nothing else ever contributes to it. The network (Mirror, \S4; live sync) only ever carries already-computed, already-verified results afterward --- never the computation itself, and never any part of it. Since identity is the keypair, not the device (\S\ref{sec:hardware-roots}'s own point), this real device can change over time --- moving to a different, real machine restores the identical real identity and its full, already-verified history there, and that new machine becomes the one computing whatever comes next; nothing about the domain's own past or claimable value depends on which real device that ever was or will be.

This is not an asymmetric cryptographic VDF (Wesolowski, Pietrzak) --- verifying costs exactly what producing costs, not asymptotically less. What it provides is unconditional: production cannot be parallelized or shortcut by any amount of hardware. It bounds the \emph{rate} at which a domain can advance its own progression; it makes no claim about calendar time. A domain with sustained, continuous compute available genuinely can advance faster than one without it --- that is the property, not a flaw in it.

Precisely because each transition is genuinely sequential, one epoch is a real mirror of real, physical elapsed time --- measured, not assumed: a real 12{,}000-iteration chain (this deployment's own \code{VDF\_ITERATIONS}) takes a real, measured $\sim$256ms of genuine wall-clock computation. No domain ever consults a clock to know this; the real, physical time simply cannot be skipped. But this mirror is a floor, not a symmetric measurement between domains: nothing stops a domain from taking longer per epoch (weaker hardware), and two domains' own real hardware for this specific computation may genuinely differ. Epoch count is therefore a real, honest measure of how much genuine sequential work \emph{this} domain has done --- correlated with, but never identical to, how much real time has elapsed for some \emph{other} domain. This is exactly why raw epoch counts across domains are never directly compared (\S\ref{sec:progression}'s own point) --- not because the physical-time mirror isn't real, but because it is calibrated differently per domain's own real hardware, and adjusting one domain's own earned value by comparing it to another's would either discount real, honest work (a slower domain penalized for real hardware limits) or reward idle time (a domain credited for simply reconnecting) --- see \S\ref{sec:causal-tick}'s own discussion of why Causal Tick stays informational, never a correction to what has actually been earned.

\section{Accrual}
\label{sec:accrual}

Once activated (\S\ref{sec:genesis}), a domain accrues claimable AIWA as a function of three quantities:
\[
R(S, t, A) = \frac{S \cdot t^{\alpha}}{\left[\beta \ln A + \ln\!\left(1 + \dfrac{C}{A^{\beta}}\right)\right]^{\gamma}}
\]

\begin{itemize}[leftmargin=1.6em]
  \item \textbf{$S$} --- committed value (from Genesis Commitment and any later commitment).
  \item \textbf{$t$} --- real progression epochs elapsed \textbf{since this domain's own last economic action} (a burn or a claim). Resets to zero on every such action.
  \item \textbf{$A$} --- this domain's own \textbf{total} progression age. Never resets.
  \item \textbf{$\alpha, \beta, \gamma, C$} --- deployment parameters.
\end{itemize}

\subsection{Why $t$ resets and $A$ does not}

This is the one precise, load-bearing distinction in the whole formula, and an earlier draft of this specification did not state it clearly enough to prevent a real implementation bug: $t$ must never be a value the claiming domain supplies. A domain that could declare its own reference epoch could always declare the earliest one available, maximizing $t$ forever. $t$ and $A$ are therefore both derived exclusively from the domain's own already-verified progression state (\S\ref{sec:progression}) at the moment of the query --- recomputed, never trusted from any event payload.
...
\subsection{The computable form}

The denominator is represented as $\beta \ln A + \ln(1 + C/A^{\beta})$ rather than $\ln(A^{\beta} + C)$ directly --- the same value ($\beta \ln A + \ln(1+C/A^{\beta}) = \ln(A^{\beta}+C)$, a real logarithmic identity), computed this way to reduce overflow risk for large $A$.
...
Identity is the keypair, never a specific piece of hardware --- losing a device, or a hardware root, is real, but never identity loss: the same real domain is restored, unchanged, from a saved key or a passphrase (\S\ref{sec:genesis}'s own \code{solana-wallet.js}) on any other real, reachable device. This closes a narrower, real problem --- a single lost or destroyed device --- but not the one \S\ref{sec:partition} itself is built around: a domain with genuinely no reachable device or channel at all, for a real, extended period (an actual communication blackout, not a broken laptop). In that case no device, old or new, has anything to reconnect to, regardless of which key it holds --- hardware roots strengthen independence assurance for a domain that IS reachable; they do not, and are not meant to, substitute for reachability itself during a real partition.
...
Sequential proof (\S6) & \code{public/core/vdf.js}, \code{public/core/wesolowski-vdf.js} (asymmetric, real cheap verification), \code{public/core/bigint-math.js} \\
Progression (\S5) & \code{public/core/progression.js} \\
Accrual formula (\S7) & \code{public/core/reward.js} \\
Accrual position, t/A (\S7.1) & \code{public/core/accrual.js} \\
Genesis Commitment (\S8) & \code{public/core/identity-cost.js}, \code{public/core/solana-wallet.js} \\
Churn profitability check (\S8) & \code{public/core/churn-analysis.js} --- a real, parameter-specific answer, never a general guarantee \\
Cross-runtime interoperability & \code{interop/rust-vdf/} --- a real, independent Rust implementation, verified byte-for-byte identical to the real JS output \\
...
Hardware roots, optional (\S13.1) & \code{public/core/hardware-attestation.js} \\
