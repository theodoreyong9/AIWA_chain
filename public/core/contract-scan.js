// Generic, reusable scanning primitives for a contract's own DAG
// events. Factored out after three separate contracts —
// generous-transfer.js, matching-contract.js, relative-rate.js —
// each wrote their own bespoke "-scan.js" reimplementing one of the
// same two patterns below. Pure, no browser dependency, no knowledge
// of what any particular contract's events mean.
 ...
 * (§15/§16). `domain`, if given, restricts to progression events for
 * that one domain. `only`, if given, restricts the result to ids
 * that are members of that set, so a caller checking a specific
 * candidate set (e.g. "which of MY sent offers") never has to filter
 * out every other domain's unrelated ids itself.
 */
